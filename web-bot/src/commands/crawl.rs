...
//! See module docs in the original for design rationale.
...
use url::Url;
use web_browser_driver::{BrowserDriver, BrowserProfileKey};
use web_crawler_db::PostgresCache;
use web_crawler_engine_v3::{
    config::{CrawlConcurrency, CrawlEngineConfig, CrawlLimits},
    input::CrawlRequest,
    policy::CrawlPolicy,
...
fn profile_strategy_from_settings(settings: &CrawlSettings) -> EngineBrowserProfileStrategy {
    ...
    match settings.profile.strategy {
        CrawlProfileStrategy::Single => EngineBrowserProfileStrategy::Single { key: fallback },
...
fn parse_text_input_line(line: &str, global_tags: &[web_crawler_db::CacheTag], parsed: &mut Vec<(Url, Vec<web_crawler_db::CacheTag>)>) {
...
fn extract_url_from_json(json: &Value, url_pointer: Option<&String>) -> anyhow::Result<Option<String>> {
...
fn print_run_header(
    ...
    eprintln!(
        ...
        settings.budget.pages, settings.budget.depth, settings.budget.total_pages, settings.budget.frontier_items
    );
    eprintln!(
        "Runtime: jobs={} sessions={} tabs={} cache-jobs={} rotate={} timeout={}s",
        settings.runtime.jobs, settings.runtime.sessions, settings.runtime.tabs,
        settings.runtime.cache_jobs, settings.runtime.rotate, settings.runtime.timeout_secs
    );
    eprintln!("Profile: strategy={:?} key={}", settings.profile.strategy, settings.profile.key);
    eprintln!(
        "Cache:   enabled={} namespace={:?} db={}",
        settings.cache.enabled, settings.cache.namespace, database_url
    );
    ...
    if !global_tags.is_empty() {
        eprintln!("Tags:    {}", global_tags.iter().map(|t| t.as_compound()).collect::<Vec<_>>().join(", "));
    }
    if !tag_pointers.is_empty() {
        eprintln!(
            "Tag pointers: {}",
            tag_pointers.iter().map(|s| format!("{}={}", s.kind, s.pointer)).collect::<Vec<_>>().join(", ")
        );
    }
}

pub async fn run(args: CrawlArgs, profile_root: &PathBuf, database_url: &str) -> anyhow::Result<()> {
    ...
    if settings.input.attach_provenance {
        tracing::warn!("--attach-provenance is currently ignored; use --tag and --tag-pointer instead");
    }
    ...
    for line in raw_lines {
        match settings.input.format {
            CrawlInputFormat::Text => parse_text_input_line(&line, &global_tags, &mut parsed),
            CrawlInputFormat::Ndjson => {
                parse_json_input_line(&line, &settings, &global_tags, &tag_pointers, &mut parsed)?;
            }
    ...
    if !settings.is_ndjson_output() {
        print_run_header(&settings, config_path.as_ref(), parsed.len(), &global_tags, &tag_pointers, database_url);
    }

    // === Postgres cache (replaces old SqliteCache) ===
    ...
    let engine: CrawlEngine<serde_json::Value> = CrawlEngine::new(
        ...
        policy,
        driver,
        postgres_cache,           // Option<PostgresCache>
        profile_root.clone(),
        profile_strategy,
    );
        ...
        for page in &result.pages {
            match &page.outcome {
                web_crawler_engine_v3::output::CrawlPageOutcome::Opened { .. } => {
                    success += 1;
                    if matches!(page.cache_decision, Some(web_crawler_engine_v3::policy::CacheDecision::Use)) {
                        from_cache += 1;
                    }
                }
                web_crawler_engine_v3::output::CrawlPageOutcome::Failed { .. } => failed += 1,
                web_crawler_engine_v3::output::CrawlPageOutcome::Skipped { .. } => skipped += 1,
            }
        }
